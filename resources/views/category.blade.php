...
@section('meta_description', $category->description ?? 'Browse all latest government jobs in ' . $category->name . ' category. Find vacancies, notifications, and application details.')
...
<div class="category-wrapper">
    ...
    <nav class="category-breadcrumb" aria-label="breadcrumb">
        ...
        <a href="{{ route('jobs') }}"><i class="fas fa-briefcase" aria-hidden="true"></i> Jobs</a>
        <span class="separator">›</span>
        <span class="current"><i class="fas fa-folder" aria-hidden="true"></i> {{ $category->name }}</span>
    </nav>
    ...
    <div class="category-header">
        <div class="category-header-content">
            ...
            <p class="subtitle">Explore all government job opportunities in the {{ $category->name }} sector</p>
    ...
    <div class="category-stats">
        ...
        <div class="stat-card-modern active">
            <div class="stat-number">{{ number_format($activeJobsCount) }}</div>
            <div class="stat-label"><i class="fas fa-clock" aria-hidden="true"></i> Active Jobs</div>
        </div>
        <div class="stat-card-modern featured">
            <div class="stat-number">{{ number_format($featuredJobsCount) }}</div>
    ...
    <div class="row">
        <div class="col-lg-8">
            ...
            <div class="section-header-modern">
                <div>
                    <h3><i class="fas fa-list" aria-hidden="true"></i> {{ $category->name }} Jobs</h3>
                    <div class="result-count">
                        Showing <strong>{{ $jobs->firstItem() }}</strong> - <strong>{{ $jobs->lastItem() }}</strong> of <strong>{{ number_format($jobs->total()) }}</strong> jobs
                ...
                <div class="sort-dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><a class="dropdown-item" href="{{ request()->fullUrlWithQuery(['sort' => 'latest']) }}"><i class="fas fa-clock" aria-hidden="true"></i> Latest First</a></li>
                        <li><a class="dropdown-item" href="{{ request()->fullUrlWithQuery(['sort' => 'oldest']) }}"><i class="fas fa-history" aria-hidden="true"></i> Oldest First</a></li>
                        <li><a class="dropdown-item" href="{{ request()->fullUrlWithQuery(['sort' => 'last_date']) }}"><i class="fas fa-calendar-alt" aria-hidden="true"></i> Last Date</a></li>
                        <li><a class="dropdown-item" href="{{ request()->fullUrlWithQuery(['sort' => 'title']) }}"><i class="fas fa-sort-alpha-up" aria-hidden="true"></i> Title A-Z</a></li>
                    </ul>
                </div>
            </div>
            ...
            <div class="job-card-modern">
                <div class="job-card-body">
                    ...
                    <div class="job-details-grid">
                        ...
                        <div class="detail-item">
                            ...
                            <strong>Qualification:</strong> {{ Str::limit($job->qualification, 60) }}
                        </div>
                        @endif
                        @if($job->job_location)
                        <div class="detail-item">
                ...
                <div class="job-card-footer">
                    <div class="footer-meta">
                        <i class="fas fa-eye" aria-hidden="true"></i> {{ $job->views ?? 0 }} views
                        @if($job->created_at)
                        ...
                        @endphp
                        Posted: {{ $createdAt->format('d M Y') }}
                        @endif
                    </div>
                    <div class="footer-actions">
                        <a href="{{ route('job.show', $job->slug) }}" class="btn-modern btn-primary-modern btn-sm-modern">
                            <i class="fas fa-info-circle" aria-hidden="true"></i> View Details
                        </a>
                        @if($job->application_link && !$isExpired)
                        <a href="{{ $job->application_link }}" target="_blank" rel="nofollow noopener noreferrer" class="btn-modern btn-success-modern btn-sm-modern">
            ...
            <div class="pagination-modern">
                <div class="info">
                    Showing <strong>{{ $jobs->firstItem() }}</strong> to <strong>{{ $jobs->lastItem() }}</strong> of <strong>{{ number_format($jobs->total()) }}</strong> entries
                </div>
            ...
            <div class="no-results-modern">
                ...
                <p>
                    @if(request()->has('search'))
                    No jobs found in <strong>{{ $category->name }}</strong> category for "<strong>{{ request('search') }}</strong>".
                    @else
                    There are no active jobs in <strong>{{ $category->name }}</strong> category at the moment.
                    @endif
                </p>
                <div class="btn-group">
                    @if(request()->has('search'))
                    <a href="{{ route('category', $category->slug) }}" class="btn-modern btn-primary-modern">
                        <i class="fas fa-sync" aria-hidden="true"></i> Show All Jobs
                    </a>
                    @endif
                    <a href="{{ route('jobs') }}" class="btn-modern btn-primary-modern">
                        <i class="fas fa-briefcase" aria-hidden="true"></i> Browse All Jobs
                    </a>
                    <a href="{{ route('home') }}" class="btn-modern btn-outline-modern">
        ...
        <div class="col-lg-4">
            ...
            <div class="sidebar-card">
                ...
                <div class="sidebar-card-body">
                    <form action="{{ route('category', $category->slug) }}" method="GET" class="search-form-modern">
                        ...
                        @if(request()->has('search'))
                        <div class="mt-2">
                            <a href="{{ route('category', $category->slug) }}" class="btn-modern btn-outline-modern btn-sm-modern">
                        ...
                        <a href="{{ route('category', $cat->slug) }}" 
                           class="list-group-item list-group-item-action d-flex justify-content-between align-items-center 
                            ...
                            <div class="d-flex align-items-center">
                                <i class="fas fa-folder me-3 {{ $cat->id == $category->id ? 'text-white' : 'text-primary' }}"></i>
                                <span>{{ $cat->name }}</span>
                            </div>
                            <span class="badge">{{ $cat->jobs_count ?? 0 }}</span>
                        </a>
                        @empty
            ...
            @php
                $featuredCategoryJobs = \App\Models\Job::where('category_id', $category->id)
                    ...
                    ->get();
            @endphp
            @if($featuredCategoryJobs->count() > 0)
            <div class="sidebar-card">
                <div class="sidebar-card-header warning">
                    <i class="fas fa-star" aria-hidden="true"></i> Featured Jobs
                </div>
                <div class="sidebar-card-body">
                    @foreach($featuredCategoryJobs as $featuredJob)
                    <div class="featured-job-item">
                        <h6>
                            <a href="{{ route('job.show', $featuredJob->slug) }}">
                                {{ Str::limit($featuredJob->title, 45) }}
                            </a>
                        </h6>
                        <div class="meta">
                    ...
                    <div class="text-center mt-3">
                        <a href="{{ route('jobs') }}?category={{ $category->id }}&featured=1" class="btn-modern btn-outline-modern btn-sm-modern">
            ...
            <div class="sidebar-card">
                ...
                <div class="sidebar-card-body">
                    <div class="quick-links-grid">
                        ...
                        <a href="{{ route('results') }}" class="btn btn-outline-success">
                            <i class="fas fa-chart-bar" aria-hidden="true"></i> Latest Results
                        </a>
                        <a href="{{ route('admit-cards') }}" class="btn btn-outline-warning">
                            <i class="fas fa-ticket-alt" aria-hidden="true"></i> Admit Cards
                        </a>
                        <a href="{{ route('answer-keys') }}" class="btn btn-outline-info">
