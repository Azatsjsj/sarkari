...
@section('title', 'Latest Government Jobs - Sarkari Results 2026')
@section('meta_description', 'Find latest Sarkari Naukri (Government Jobs) notifications, vacancies, and employment opportunities from various departments like SSC, UPSC, Railway, Bank, Police, and more.')
...
<div class="sarkari-container">
    ...
    <div class="search-box">
        ...
        <form action="{{ route('jobs') }}" method="GET" role="search" aria-label="Filter jobs">
            <div class="filter-group">
                    ...
                    <select name="category" class="form-control" aria-label="Filter by job category">
                        ...
                        @foreach($categories ?? [] as $category)
                            <option value="{{ $category->id }}" 
                                ...
                                {{ $category->name }} ({{ $category->jobs_count ?? $category->jobs->count() ?? 0 }})
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="filter-actions">
                    ...
                    <a href="{{ route('jobs') }}" class="btn-secondary-modern">
        ...
        <div class="section-content">
            <div class="featured-grid">
                ...
                <div class="featured-card">
                    ...
                    <div class="job-title">
                        <a href="{{ route('job.show', $job->slug ?? $job->id) }}">{{ \Illuminate\Support\Str::limit($job->title, 55) }}</a>
                    </div>
                    <div class="job-meta">
                        <span><i class="fas fa-building" aria-hidden="true"></i> {{ $job->category->name ?? 'N/A' }}</span>
                        <span><i class="fas fa-users" aria-hidden="true"></i> {{ $job->total_post ?? 'N/A' }} Posts</span>
                        <span><i class="fas fa-calendar-alt" aria-hidden="true"></i> Last: {{ $lastDate->format('d M Y') }}</span>
                    </div>
                    @if(!empty($job->short_description))
                    <div class="job-desc">
                        {{ \Illuminate\Support\Str::limit($job->short_description, 80) }}
                    </div>
                    @endif
                    <div style="margin-top: 10px; display: flex; gap: 10px; flex-wrap: wrap;">
                        <a href="{{ route('job.show', $job->slug ?? $job->id) }}" class="view-link">
        ...
        <div class="section-content p-0">
            ...
            <div class="job-item">
                <div class="job-row">
                    <div class="job-info">
                        <div class="job-title">
                            <a href="{{ route('job.show', $job->slug ?? $job->id) }}">
                                {{ $job->title }}
                            </a>
                        ...
                        <div class="job-meta">
                            <span><i class="fas fa-building" aria-hidden="true"></i> {{ $job->category->name ?? 'N/A' }}</span>
                            <span><i class="fas fa-users" aria-hidden="true"></i> {{ $job->total_post ?? 'N/A' }} Posts</span>
                            <span><i class="fas fa-calendar-check" aria-hidden="true"></i> Start: {{ $startDate->format('d M Y') }}</span>
                        ...
                        @if(!empty($job->short_description))
                        <div class="job-desc">
                            {{ \Illuminate\Support\Str::limit($job->short_description, 120) }}
                        </div>
                        @endif
                        @if(!empty($job->qualification))
                        <div class="job-meta">
                            <span><i class="fas fa-graduation-cap" aria-hidden="true"></i> Qualification: {{ \Illuminate\Support\Str::limit($job->qualification, 140) }}</span>
                        </div>
                        @endif
                    </div>
                    <div class="job-actions">
                        ...
                        @if(!empty($job->application_link) && !$isExpired)
                        ...
                        @if(!empty($job->application_fee))
                            <span class="badge-modern badge-category" style="margin-top: 4px;">
                                <i class="fas fa-money-bill" aria-hidden="true"></i> Fee: {{ $job->application_fee }}
                            </span>
                        @endif
                    </div>
                </div>
            ...
            <div class="no-results">
                ...
                <a href="{{ route('jobs') }}" class="btn-primary-modern" style="display: inline-block;">
    ...
    <div class="section-box">
        ...
        <div class="section-content">
            <div class="categories-grid">
                @foreach($categories as $category)
                <a href="{{ route('category', $category->slug ?? $category->id) }}" class="category-item">
                    <div class="cat-icon">
                        <i class="fas fa-folder-open" aria-hidden="true"></i>
                    </div>
                    <div class="cat-name">{{ \Illuminate\Support\Str::limit($category->name, 20) }}</div>
                    <div class="cat-count">{{ number_format($category->jobs_count ?? $category->jobs->count() ?? 0) }} Jobs</div>
                </a>
                @endforeach
            </div>
        </div>
