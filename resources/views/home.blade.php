...
@push('styles')
    {{-- Unique, modern CSS with a fresh color palette and improved UI/UX --}}
    <style>
        /* -------------------- Base & Reset -------------------- */
        * {
...
@section('content')
    <div class="container mt-2 mt-md-3">
            ...
            <div class="marquee-modern shadow-soft">
                <div class="container px-2">
                    <marquee behavior="alternate" onmouseover="this.stop();" onmouseout="this.start();">
                        @if(isset($marqueeJobs))
                            @foreach($marqueeJobs as $job)
                                <a href="{{ route('job.show', $job->slug) }}">{{ $job->title }} Apply Online {{ date('Y') }}</a> ||
                            @endforeach
                        @endif
                        @if(isset($marqueeResults))
                            @foreach($marqueeResults as $result)
                                <a href="{{ route('results.show', $result->slug) }}">{{ $result->title }} Coming Soon</a> ||
                            @endforeach
                        @endif
                        @if(isset($marqueeAdmitCards))
                            @foreach($marqueeAdmitCards as $admitCard)
                                <a href="{{ $admitCard->slug ? route('admit-card.show', $admitCard->slug) : route('admit-cards') }}">{{ $admitCard->title }} Admit Card</a> ||
                            @endforeach
                        @endif
                        @if(isset($marqueeAnswerKeys))
                            @foreach($marqueeAnswerKeys as $answerKey)
                                <a href="{{ route('answer-key.show', $answerKey->slug) }}">{{ $answerKey->title }} Answer Key</a> ||
                            @endforeach
                        @endif
                        @if(isset($marqueeAdmissions))
                            @foreach($marqueeAdmissions as $admission)
                                <a href="{{ route('admissions.show', $admission->slug) }}">{{ $admission->title }} Admissions {{ date('Y') }}</a>
                                @if(!$loop->last) || @endif
                            @endforeach
                        @endif
                    </marquee>
        ...
        <div class="quick-grid">
            ...
            @foreach($allItems as $item)
                ...
                @endphp
                <a href="{{ $route }}" class="quick-card">
                    <i class="fas {{ $item['icon'] }}"></i>
                    <span>{{ Str::limit($data->title, 45) }}</span>
                </a>
            @endforeach
        </div>
        ...
        <div class="two-col-grid">
            ...
            <div class="card-modern card">
                <div class="card-header-jobs d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('jobs') }}?featured=1" class="btn btn-light btn-sm rounded-pill px-3">View All <i class="fas fa-arrow-right ms-1"></i></a>
                </div>
                        ...
                        @forelse($featuredJobs as $job)
                            <div class="list-item-modern">
                                <div class="d-flex flex-wrap align-items-start justify-content-between gap-1">
                                    <a href="{{ route('job.show', $job->slug) }}" class="item-title">{{ Str::limit($job->title, 55) }}</a>
                                    <div class="d-flex gap-1 flex-wrap">
                                        <span class="badge-soft badge-soft-warning"><i class="fas fa-star me-1"></i>Featured</span>
                                        @if(isset($job->last_date) && \Carbon\Carbon::parse($job->last_date)->isPast())
                                            <span class="badge-soft badge-soft-danger">Expired</span>
                                        @elseif(isset($job->last_date) && \Carbon\Carbon::parse($job->last_date)->diffInDays(now()) <= 3)
                                ...
                                <div class="item-meta mt-1">
                                    ...
                                    @if(isset($job->last_date))
                                        <span><i class="fas fa-calendar-alt"></i> Last: <span class="{{ \Carbon\Carbon::parse($job->last_date)->isPast() ? 'text-danger' : 'text-success' }}">{{ \Carbon\Carbon::parse($job->last_date)->format('d M Y') }}</span></span>
                                    @endif
                                    @if(isset($job->total_post))
            ...
            <div class="card-modern card">
                <div class="card-header-jobs d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('jobs') }}" class="btn btn-light btn-sm rounded-pill px-3">View All <i class="fas fa-arrow-right ms-1"></i></a>
                </div>
                <div class="card-body p-0">
                    @if(isset($latestJobs) && $latestJobs->count())
                        @forelse($latestJobs as $job)
                            <div class="list-item-modern">
                                <div class="d-flex flex-wrap align-items-start justify-content-between gap-1">
                                    <a href="{{ route('job.show', $job->slug) }}" class="item-title">{{ Str::limit($job->title, 55) }}</a>
                                    @if(isset($job->is_featured) && $job->is_featured)
                                        <span class="badge-soft badge-soft-warning"><i class="fas fa-star me-1"></i>Featured</span>
                                    @endif
                                </div>
                                <div class="item-meta mt-1">
            ...
            <div class="card-modern card">
                <div class="card-header-admit d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('admit-cards') }}" class="btn btn-light btn-sm rounded-pill px-3">View All <i class="fas fa-arrow-right ms-1"></i></a>
                </div>
                <div class="card-body p-0">
                    @if(isset($latestAdmitCards) && $latestAdmitCards->count())
                        @foreach($latestAdmitCards as $admit)
                            <div class="list-item-modern">
                                <a href="{{ $admit->slug ? route('admit-card.show', $admit->slug) : route('admit-cards') }}" class="item-title d-block">{{ Str::limit($admit->title, 50) }}</a>
                                <div class="item-meta mt-1">
                                    <span><i class="fas fa-building"></i> {{ $admit->job->title ?? 'N/A' }}</span>
                                    @if(isset($admit->admit_card_date))
                                        <span><i class="fas fa-calendar"></i> {{ safe_date_format($admit->admit_card_date, 'd M Y') }}</span>
                                    @endif
                                    @if(isset($admit->download_link))
                                        <a href="{{ htmlspecialchars($admit->download_link) }}" target="_blank" rel="noopener" class="badge-soft badge-soft-success text-decoration-none"><i class="fas fa-download me-1"></i>Download</a>
                                    @endif
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="card-modern card">
                <div class="card-header-result d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('results') }}" class="btn btn-light btn-sm rounded-pill px-3">View All <i class="fas fa-arrow-right ms-1"></i></a>
                </div>
                <div class="card-body p-0">
                        ...
                        @forelse($latestResults as $result)
                            <div class="list-item-modern">
                                <div class="d-flex flex-wrap justify-content-between align-items-start gap-1">
                                    <a href="{{ route('results.show', $result->slug) }}" class="item-title">{{ Str::limit($result->title, 50) }}</a>
                                    <span class="badge-soft badge-soft-success"><i class="fas fa-newspaper me-1"></i>New</span>
                                </div>
                                <div class="item-meta mt-1">
                                    <span><i class="fas fa-calendar"></i> {{ isset($result->result_date) ? \Carbon\Carbon::parse($result->result_date)->format('d M Y') : 'N/A' }}</span>
                                    <a href="{{ route('results.show', $result->slug) }}" class="text-primary text-decoration-none fw-medium">Check Result <i class="fas fa-arrow-right ms-1"></i></a>
                                </div>
                            </div>
                        @empty
            ...
            <div class="card-modern card">
                <div class="card-header-answer d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('answer-keys') }}" class="btn btn-light btn-sm rounded-pill px-3">View All <i class="fas fa-arrow-right ms-1"></i></a>
                </div>
                <div class="card-body p-0">
                    @if(isset($latestAnswerKeys) && $latestAnswerKeys->count())
                        @forelse($latestAnswerKeys as $key)
                            <div class="list-item-modern">
                                <div class="d-flex flex-wrap justify-content-between align-items-start gap-1">
                                    <a href="{{ route('answer-key.show', $key->slug) }}" class="item-title">{{ Str::limit($key->title, 50) }}</a>
                                    <span class="badge-soft badge-soft-primary"><i class="fas fa-key me-1"></i>Answer Key</span>
                                </div>
                                <div class="item-meta mt-1">
                                    <span><i class="fas fa-calendar"></i> {{ isset($key->answer_key_date) ? \Carbon\Carbon::parse($key->answer_key_date)->format('d M Y') : 'N/A' }}</span>
                                    <a href="{{ route('answer-key.show', $key->slug) }}" class="text-primary text-decoration-none fw-medium">View Key <i class="fas fa-arrow-right ms-1"></i></a>
                                </div>
                            </div>
                        @empty
            ...
            {{-- COL 6: Documents (Full width in grid) --}}
            <div class="card-modern card" style="grid-column: 1 / -1;">
                <div class="card-header-doc d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <a href="{{ route('documents.index') }}" class="btn btn-light btn-sm rounded-pill px-3 fw-semibold shadow-sm text-primary">View All <i class="fas fa-arrow-right ms-1"></i></a>
                </div>
                <div class="card-body p-3 p-md-4">
                    @if(isset($featuredDocuments) && $featuredDocuments->count())
                        <div class="row g-3">
                                    ...
                                    <div class="document-card-modern">
                                        <div class="d-flex align-items-start gap-3">
                                            <div class="doc-icon-badge">
                                                <i class="fas {{ $doc->getFileIcon() ?? 'fa-file-alt' }}"></i>
                                            </div>
                                            <div class="flex-grow-1">
                                                <a href="{{ route('documents.show', $doc->slug) }}" class="doc-title-link mb-1">{{ Str::limit($doc->title, 55) }}</a>
                                            </div>
                                        </div>
                                        <div class="d-flex flex-wrap gap-1 mt-auto pt-2 border-top">
                                            @if(isset($doc->type))
                                                <span class="doc-meta-tag {{ $doc->type == 'notice' ? 'doc-tag-notice' : ($doc->type == 'certificate' ? 'doc-tag-certificate' : ($doc->type == 'syllabus' ? 'doc-tag-syllabus' : 'doc-tag-result')) }}">
                                                    @if($doc->type == 'notice') <i class="fas fa-bullhorn"></i> Notice
                                                    @elseif($doc->type == 'certificate') <i class="fas fa-certificate"></i> Certificate
                                                    @elseif($doc->type == 'syllabus') <i class="fas fa-book"></i> Syllabus
                                                    @elseif($doc->type == 'result') <i class="fas fa-chart-line"></i> Result
                                                    @endif
                                                </span>
                                            @endif
                                            @if(isset($doc->department))
                                                <span class="doc-meta-tag doc-tag-department"><i class="fas fa-building"></i> {{ Str::limit($doc->department, 18) }}</span>
                                            @endif
                                            @if(isset($doc->issue_date))
                                                <span class="doc-meta-tag doc-tag-date"><i class="fas fa-calendar-alt"></i> {{ safe_date_format($doc->issue_date, 'd M Y') }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                    ...
                    @if(isset($latestDocuments) && $latestDocuments->count() > ($featuredDocuments->count() ?? 0))
                        <hr class="my-4">
                        ...
                        <div class="list-group list-group-flush">
                            @foreach($latestDocuments->take(4) as $doc)
                                <a href="{{ route('documents.show', $doc->slug) }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center px-2 border-0 border-bottom py-2 rounded">
                                    <span><i class="fas {{ $doc->getFileIcon() ?? 'fa-file-alt' }} me-2 text-primary"></i>{{ Str::limit($doc->title, 55) }}</span>
                    ...
                    <div class="text-center mt-4">
                        <a href="{{ route('documents.index') }}" class="btn btn-outline-primary rounded-pill px-4 fw-semibold">View All Government Documents &amp; Notices <i class="fas fa-arrow-right ms-2"></i></a>
                    </div>
                </div>
            </div>

            {{-- COL 7: Quick Links Sidebar --}}
            <div class="card-modern card">
                ...
                <div class="card-body p-3">
                    <div class="d-grid gap-2">
                        <a href="{{ route('jobs') }}" class="btn btn-outline-primary text-start py-2 px-3 rounded-pill"><i class="fas fa-briefcase me-2"></i>Browse All Jobs</a>
                        <a href="{{ route('results') }}" class="btn btn-outline-success text-start py-2 px-3 rounded-pill"><i class="fas fa-chart-bar me-2"></i>Check Results</a>
                        <a href="{{ route('admit-cards') }}" class="btn btn-outline-info text-start py-2 px-3 rounded-pill"><i class="fas fa-ticket-alt me-2"></i>Admit Cards</a>
                        <a href="{{ route('answer-keys') }}" class="btn btn-outline-warning text-start py-2 px-3 rounded-pill"><i class="fas fa-key me-2"></i>Answer Keys</a>
                        <a href="{{ route('jobs') }}?status=active" class="btn btn-outline-secondary text-start py-2 px-3 rounded-pill"><i class="fas fa-clock me-2"></i>Active Jobs</a>
                        <a href="{{ route('results') }}?filter=upcoming" class="btn btn-outline-danger text-start py-2 px-3 rounded-pill"><i class="fas fa-bell me-2"></i>Upcoming Results</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        {{-- ========== FEATURED ADMISSIONS ========== --}}
        @if(isset($featuredAdmissions) && $featuredAdmissions->count())
            <div class="row mt-4">
                <div class="col-12">
                    <div class="card-modern card">
                        <div class="card-header" style="background:linear-gradient(145deg,#4f2d5e,#2d1a3a); color:#fff; border:none; border-radius:16px 16px 0 0; padding:0.9rem 1.2rem;">
                            <div class="d-flex justify-content-between align-items-center flex-wrap">
                                ...
                                <a href="{{ route('admissions') }}?featured=1" class="btn btn-light btn-sm rounded-pill px-3">View All <i class="fas fa-arrow-right ms-1"></i></a>
                            </div>
                        </div>
                        <div class="card-body p-3">
                            <div class="row g-3">
                                @foreach($featuredAdmissions as $admission)
                                    <div class="col-xl-3 col-lg-4 col-md-6">
                                        <div class="card h-100 border-0 shadow-soft shadow-hover border-radius-12">
                                            <div class="card-header bg-transparent border-0 d-flex justify-content-between align-items-center px-3 pt-3 pb-0">
                                                <span class="badge-soft badge-soft-warning"><i class="fas fa-star me-1"></i>Featured</span>
                                                @if(isset($admission->last_date) && \Carbon\Carbon::parse($admission->last_date)->isPast())
                                                    <span class="badge-soft badge-soft-danger">Expired</span>
                                                @elseif(isset($admission->last_date) && \Carbon\Carbon::parse($admission->last_date)->diffInDays(now()) <= 3)
                                                    <span class="badge-soft badge-soft-danger">Urgent</span>
                                                @endif
                                            </div>
                                            <div class="card-body px-3 pt-2">
                                                <h6 class="fw-semibold text-primary">{{ Str::limit($admission->title, 40) }}</h6>
                                                @if(isset($admission->university))
                                                    <p class="text-muted small mb-1"><i class="fas fa-university me-1"></i>{{ Str::limit($admission->university->name ?? 'N/A', 20) }}</p>
                                                @endif
                                                @if(isset($admission->course))
                                                    <p class="text-muted small mb-1"><i class="fas fa-graduation-cap me-1"></i>{{ Str::limit($admission->course->name ?? 'N/A', 20) }}</p>
                                                @endif
                                                @if(isset($admission->last_date))
                                                    <p class="text-muted small mb-0"><i class="fas fa-clock me-1"></i>Last: <span class="{{ \Carbon\Carbon::parse($admission->last_date)->isPast() ? 'text-danger' : 'text-success' }}">{{ \Carbon\Carbon::parse($admission->last_date)->format('d M Y') }}</span></p>
                                                @endif
                                            </div>
                                            <div class="card-footer bg-transparent border-0 d-flex gap-2 px-3 pb-3 pt-0">
                                                <a href="{{ route('admissions.show', $admission->slug) }}" class="btn btn-outline-primary btn-sm flex-fill rounded-pill"><i class="fas fa-eye me-1"></i>View</a>
                                                @if(isset($admission->apply_url) && isset($admission->last_date) && !\Carbon\Carbon::parse($admission->last_date)->isPast())
                                                    <a href="{{ $admission->apply_url }}" target="_blank" rel="noreferrer" class="btn btn-success btn-sm flex-fill rounded-pill"><i class="fas fa-external-link-alt me-1"></i>Apply</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
        ...
        <div class="row my-4">
            <div class="col-12">
                <form action="{{ route('jobs') }}" method="GET" class="search-wrapper d-flex align-items-center">
                    <i class="fas fa-search text-muted me-2"></i>
                    <input type="text" name="search" class="flex-grow-1" placeholder="Search jobs, categories, keywords..." value="{{ request('search') }}">
        ...
        <div class="row g-3 mb-4">
            <div class="col-6 col-md-3"><div class="stat-card"><div class="stat-number">{{ $totalJobs ?? 0 }}</div><div class="stat-label"><i class="fas fa-briefcase me-1"></i>Total Jobs</div></div></div>
            <div class="col-6 col-md-3"><div class="stat-card"><div class="stat-number">{{ $totalResults ?? 0 }}</div><div class="stat-label"><i class="fas fa-chart-bar me-1"></i>Results</div></div></div>
            <div class="col-6 col-md-3"><div class="stat-card"><div class="stat-number">{{ $activeJobs ?? 0 }}</div><div class="stat-label"><i class="fas fa-clock me-1"></i>Active Jobs</div></div></div>
            <div class="col-6 col-md-3"><div class="stat-card"><div class="stat-number">{{ $upcomingResults ?? 0 }}</div><div class="stat-label"><i class="fas fa-bell me-1"></i>Upcoming</div></div></div>
        </div>
                    ...
                    <div class="col-lg-7">
                        <div class="card h-100 border-0 shadow-sm">
                            <div class="card-body">
                                ...
                                <div class="mt-3"><a href="{{ route('jobs') }}" class="btn-primary-custom"><i class="fas fa-search"></i> Explore latest vacancies</a></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-5">
    ...
    @if(View::exists('components.maincantent'))
        <x-maincantent />
    @endif
@endsection
