{{-- resources/views/admit-cards/index.blade.php --}}
@extends('layouts.app')

@section('title', 'Latest Admit Cards - Sarkari Result 2026')
@section('meta_description', 'Download latest admit cards for various government exams. Get hall tickets, exam dates, venues, and important instructions for SSC, UPSC, Railway, Bank, Police and more.')
...
<div class="sarkari-container">
    ...
    <div class="search-box">
        ...
        <form action="{{ route('admit-cards') }}" method="GET">
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                ...
                <div style="flex: 1; min-width: 150px;">
                    <select name="filter" class="form-control" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
                        ...
                        <option value="upcoming" {{ request('filter') == 'upcoming' ? 'selected' : '' }}>Upcoming Exams</option>
                    </select>
                </div>
                <div>
                    ...
                    <a href="{{ route('admit-cards') }}" style="background: #6c757d; color: #fff; padding: 10px 15px; border-radius: 5px; text-decoration: none; display: inline-block;">
    ...
    <div class="stats-row">
        <div class="stat-card">
            <div class="stat-number">{{ $admitCards->total() ?? 0 }}</div>
    ...
    <div class="row" style="display: flex; gap: 20px; flex-wrap: wrap;">
        ...
        <div style="flex: 2.5; min-width: 280px;">
            ...
            @if(isset($admitCards) && $admitCards->count() > 0)
            <div class="section-box">
                ...
                <div class="section-content">
                    ...
                    <div style="padding: 10px 15px; background: #f8f9fa; border-bottom: 1px solid #e0e0e0; font-size: 13px;">
                        <i class="fas fa-info-circle"></i> Showing {{ $admitCards->firstItem() }} - {{ $admitCards->lastItem() }} of {{ $admitCards->total() }} admit cards
                    </div>
                        ...
                        <div class="row" style="display: flex; flex-wrap: wrap;">
                            <div style="flex: 2; min-width: 200px;">
                                ...
                                <div class="admit-card-meta">
                                    ...
                                    <span><i class="fas fa-building"></i> {{ $admitCard->job->category->name }}</span>
                                    @endif
                                    @if($admitCard->job)
                                    <span><i class="fas fa-briefcase"></i> {{ Str::limit($admitCard->job->title, 40) }}</span>
                                    @endif
                                    <span><i class="fas fa-calendar-alt"></i> Admit Card: {{ safe_date_format($admitCardDate, 'd M Y') }}</span>
                                </div>
                                ...
                                <div class="admit-card-desc">
                                    {{ Str::limit($admitCard->short_description, 100) }}
                                </div>
                                @endif
                                <div class="admit-card-meta">
                                    <span><i class="fas fa-eye"></i> Views: {{ $admitCard->views ?? 0 }}</span>
                                    <span><i class="fas fa-download"></i> Downloads: {{ $admitCard->download_count ?? 0 }}</span>
                                    @if($admitCard->exam_venue)
                                    <span><i class="fas fa-map-marker-alt"></i> {{ Str::limit($admitCard->exam_venue, 30) }}</span>
                                    @endif
                                </div>
                            </div>
                            <div style="flex: 1; min-width: 180px; text-align: right;">
                                <div style="margin-bottom: 8px;">
                                    ...
                                    <a href="{{ route('admit-card.download', $admitCard->slug ?? $admitCard->id) }}" 
                                       ...
                                       onclick="trackDownload({{ $admitCard->id }})">
                                        <i class="fas fa-download"></i> Download Admit Card
                                    </a>
                                    @elseif(isset($admitCard->download_link) && $admitCard->download_link)
                                    <a href="{{ $admitCard->download_link }}" target="_blank" class="download-link">
                                        <i class="fas fa-external-link-alt"></i> Download Admit Card
                                    </a>
                                    @endif
                                </div>
                                <div>
                                    <a href="{{ $admitCard->slug ? route('admit-card.show', $admitCard->slug) : route('admit-cards') }}" class="view-link">
            ...
            <div class="section-box">
                ...
                <div class="no-results">
                    ...
                    <p style="color: #666;">
                        @if(request()->has('search'))
                        No admit cards found for "{{ request('search') }}". Try different keywords or browse all admit cards.
                        @else
                    ...
                    <div style="display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                        @if(request()->has('search'))
                        <a href="{{ route('admit-cards') }}" style="background: #ffc107; color: #000; padding: 10px 20px; border-radius: 5px; text-decoration: none;">
        ...
        <div style="flex: 1; min-width: 250px;">
            ...
            <div class="sidebar-box">
                ...
                <div class="sidebar-content">
                    @forelse(($recentAdmitCards ?? []) as $recent)
                    <div class="recent-item">
                        <div class="recent-title">
                            <a href="{{ $recent->slug ? route('admit-card.show', $recent->slug) : route('admit-cards') }}">
            ...
            <!-- Upcoming Exams -->
            @if(isset($upcomingAdmitCards) && $upcomingAdmitCards->count() > 0)
            <div class="sidebar-box">
                <div class="sidebar-header" style="background: #28a745; color: #fff;">
                    <i class="fas fa-bell"></i> Upcoming Exams
                </div>
                <div class="sidebar-content">
                    @foreach($upcomingAdmitCards as $upcoming)
                    <div class="recent-item">
                        <div class="recent-title">
                            <a href="{{ $upcoming->slug ? route('admit-card.show', $upcoming->slug) : route('admit-cards') }}">
                        ...
                        <div class="recent-date">
                            ...
                            @endphp
                            Exam: {{ $upcomingDate->format('d M Y') }}
                        </div>
                    </div>
                    @endforeach
                </div>
